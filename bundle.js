var MemoryGame = (function () {
  ...
  function isTurnEnd(game) {
      var revealed = filter(game, { revealed: true,
          solved: false
      });
  ...
  function main$1 () {
      ...
      function getSelection() {
          return select(config.ids.root)
              .selectAll("." + config.classes.card);
      }
      function build(selection$$1, data, delay) {
          if (delay) {
              setTimeout(build.bind(null, selection$$1, data), delay);
              return;
          }
          data = data || selection$$1.data();
          forEach(data, function (d) {
              d.solved = false;
              d.revealed = false;
          });
          data = shuffle$2(data);
          selection$$1.data(data)
              .call(layout)
              .call(draw);
      }
      function layout(selection$$1) {
          var added = selection$$1.enter()
              .append("div")
      ...
      function draw(selection$$1, delay) {
          if (delay) {
              setTimeout(draw.bind(null, selection$$1), delay);
              return;
          }
          selection$$1.classed(config.classes.solved, function (d) { return d.solved; });
          selection$$1.classed(config.classes.revealed, function (d) { return d.revealed; });
          selection$$1.classed(config.classes.shuffling, false);
          selection$$1.select("." + config.classes.front)
              .text(function (d) {
              return d.value;
          })
      ...
      function reset(selection$$1, delay) {
          if (delay) {
              setTimeout(reset.bind(null, selection$$1), delay);
              return;
          }
          selection$$1.classed(config.classes.shuffling, true);
          // give the CSS animation time to finish
          selection$$1.call(build, null, config.tick);
      }
      function onClick(card) {
          var selection$$1 = getSelection();
          var move = revealCard(selection$$1.data(), card);
          if (move.success) {
              // paint reveal immediately then check if we can end the turn
              select(this).call(draw);
              if (move.endTurn) {
                  selection$$1.call(update);
              }
          }
      }
      function update(selection$$1) {
          var turn = endTurn(selection$$1.data());
          if (turn.match) {
              selection$$1.call(draw);
          }
          else {
              selection$$1.call(draw, config.tick); // wait a tick before hiding a failed attempt
          }
          if (turn.solved) {
              selection$$1.call(reset, config.tick); // wait a tick before reseting the whole board
          }
      }
  }
